...
class GDN(nn.Module):
    """PyTorch implementation of TensorFlow Compression's GDN/IGDN layer."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # TensorFlow GDN uses gamma[j, i] for output channel i. PyTorch Conv2d
        # expects weight[out_channel, in_channel, ...], so we transpose first.
...
class SignalConv2D(nn.Module):
    """
    Minimal PyTorch port of tfc.SignalConv2D for the cases used in CNN_img.py.

    Supported combinations here:
    - corr=True, strides_down>=1, strides_up=1
    - corr=False, strides_up>=1, strides_down=1
    - padding="valid" or padding="same_zeros"
    """
    ...
    def __init__(
        ...
        if any(stride != 1 for stride in self.strides_up) and any(
            stride != 1 for stride in self.strides_down
        ):
            raise NotImplementedError(
                "This port does not support simultaneous upsampling and downsampling."
            )

        if self.padding not in ("valid", "same_zeros"):
            raise NotImplementedError(
                "Only 'valid' and 'same_zeros' padding are implemented."
            )

        if self.corr:
            if any(stride != 1 for stride in self.strides_up):
                raise NotImplementedError(
                    "Cross-correlation with strides_up is not used in CNN_img.py."
                )
            self.op = nn.Conv2d(
            ...
            if any(stride != 1 for stride in self.strides_down):
                raise NotImplementedError(
                    "Convolution with strides_down is not used in CNN_img.py."
                )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.corr and self.padding == "same_zeros":
            padding_pairs = _same_padding_for_kernel(
                self.kernel_size,
                corr=True,
                strides_up=(1, 1),
            )
...
class MVAnalysis(nn.Module):
    """PyTorch equivalent of CNN_img.MV_analysis."""

    def __init__(self, num_filters: int, M: int, in_channels: int = 2):
        super().__init__()
...
class MVSynthesis(nn.Module):
    """PyTorch equivalent of CNN_img.MV_synthesis."""

    def __init__(self, num_filters: int, M: int, out_channels: int = 2):
        super().__init__()
        self.layer_0 = SignalConv2D(
            M,
            num_filters,
...
class ResAnalysis(nn.Module):
    """PyTorch equivalent of CNN_img.Res_analysis."""

    def __init__(self, num_filters: int, M: int, in_channels: int = 3):
        super().__init__()
        self.layer_0 = SignalConv2D(
            in_channels,
            num_filters,
...
class ResSynthesis(nn.Module):
    """PyTorch equivalent of CNN_img.Res_synthesis."""

    def __init__(self, num_filters: int, M: int, out_channels: int = 3):
        super().__init__()
        self.layer_0 = SignalConv2D(
            M,
            num_filters,
...
# Keep aliases close to the TensorFlow names from CNN_img.py.
